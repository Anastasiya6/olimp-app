...
class EntryDetailDesignationService
{
    ...
    public function entryDetailDesignation($designation,$department)
    {
        ...
        $this->pdf = PDFService::getPdf($this->header1,$this->header2,$this->width,'Комплектовочна відомість','Деталь '.$designation->designation.  ' ЦЕХ '.$this->department);
        ...
        $this->node($designation->id,0,1,1);
    ...
    public function node($designation_id,$level,$quantity_node,$route=null,$designation='',$designation_name='',$pred_quantity_node=0)
    {
        //$this->newList();
        ...
        if ($specifications->isNotEmpty()) {
            ...
            if($route == null || $route == $this->department) {
                if ($level == 1) {
                    $this->pdf->MultiCell($this->width[0], $this->height, '', 0, 'L', 0, 0, '', '', true, 0, false, true, $this->max_height, 'T');
            ...
            foreach ($specifications as $specification) {
                ...
                $this->node($specification->designation_entry_id, 1, $specification->quantity, substr($specification->designationEntry->route, 0, 2), $specification->designationEntry->designation, $specification->designationEntry->name, $quantity_node);
