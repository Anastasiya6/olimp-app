        ...
        <div class="mx-auto max-w-5xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route($route.'.update',$material->id) }}">
                        @csrf
                        @method('put')
                        <div class="mb-6">
                            <label class="block">
                                ...
                                <input type="text" name="name"
                                       ...
                                       placeholder="" value="{{old('name',$material->name)}}" />
                            </label>
                            @error('name')
                                ...
                                <select name="type_unit_id" class="block w-full mt-1 rounded-md">
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->id }}"
                                            @if($material->type_unit_id == $unit->id) selected @endif 'selected' }}>
                                            {{ $unit->unit }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-6">
                            <label class="block">
                                ...
                                <input type="text" name="code_1c" class="block w-full mt-1 rounded-md" placeholder=""
                                       value="{{ old('code_1c',$material->code_1c) }}" />
                            </label>
                            @error('code_1c')
                            <div class="text-sm text-red-600">{{ $message }}</div>
                            @enderror
